import type { AIProvider, AIProviderType } from "@/core/ai/contracts";
...
export function createProviderAdapter(input: {
  ...
  if (input.type === "anthropic") return new AnthropicAdapter(input.baseUrl, input.apiKey);
  if (input.type === "gemini") return new GeminiAdapter(input.baseUrl, input.apiKey);
