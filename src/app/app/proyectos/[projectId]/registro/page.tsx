  ...
  return (
    <div className="mx-auto max-w-7xl pb-20 lg:pb-0">
      ...
      <section className="mt-5 grid gap-4 xl:grid-cols-2">
        <article className="nexus-panel rounded-2xl p-5 sm:p-6"><div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><Gavel className="size-4 text-violet-300/70" /><div className="nexus-kicker">Decisiones</div></div><h2 className="mt-2 text-base font-semibold text-slate-100">Historial arquitectónico</h2></div><span className="font-mono text-xs text-primary">{records.decisions.length}</span></div><div className="mt-5 space-y-3">{records.decisions.map((record) => <div key={record.id} className="rounded-xl border border-white/[0.055] bg-black/10 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-semibold text-slate-200">{record.title}</h3><span className="rounded-full border border-white/[0.06] px-2 py-1 text-[0.58rem] text-slate-500">{record.status}</span></div><div className="mt-3"><MessageMarkdown content={record.decision} compact /></div>{record.agent ? <div className="mt-3 text-[0.62rem] text-slate-600">Registrada con {record.agent.name}</div> : null}</div>)}{!records.decisions.length ? <p className="text-sm text-slate-600">Todavía no hay decisiones registradas.</p> : null}</div></article>
        <article className="nexus-panel rounded-2xl p-5 sm:p-6"><div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><Bug className="size-4 text-rose-300/70" /><div className="nexus-kicker">Errores y soluciones</div></div><h2 className="mt-2 text-base font-semibold text-slate-100">Base de causas raíz</h2></div><span className="font-mono text-xs text-primary">{records.errors.length}</span></div><div className="mt-5 space-y-3">{records.errors.map((record) => <div key={record.id} className="rounded-xl border border-white/[0.055] bg-black/10 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-semibold text-slate-200">{record.title}</h3><span className="rounded-full border border-white/[0.06] px-2 py-1 text-[0.58rem] text-slate-500">{record.status}</span></div>{record.error_signature ? <pre className="nexus-scrollbar mt-3 overflow-x-auto rounded-lg bg-black/25 p-3 font-mono text-[0.65rem] text-rose-100/60">{record.error_signature}</pre> : null}<div className="mt-3"><MessageMarkdown content={record.solution} compact /></div>{record.agent ? <div className="mt-3 text-[0.62rem] text-slate-600">Registrado con {record.agent.name}</div> : null}</div>)}{!records.errors.length ? <p className="text-sm text-slate-600">Todavía no hay errores documentados.</p> : null}</div></article>
      </section>
    </div>
  );
}
