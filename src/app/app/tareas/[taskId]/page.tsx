  ...
  return (
    <div className="mx-auto max-w-7xl pb-20 lg:pb-0">
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1fr_20rem]">
        <article className="nexus-panel rounded-2xl p-5 sm:p-6"><div className="nexus-kicker">Alcance</div><h2 className="mt-2 text-base font-semibold text-foreground">Descripción</h2><div className="mt-4"><MessageMarkdown content={taskRecord.description || "Sin descripción."} /></div>{taskRecord.acceptance_criteria ? <><h2 className="mt-7 border-t border-border pt-5 text-base font-semibold text-foreground">Criterios de aceptación</h2><div className="mt-4"><MessageMarkdown content={taskRecord.acceptance_criteria} /></div></> : null}</article>
        <aside className="space-y-3">
          <article className="nexus-panel rounded-2xl p-5"><div className="flex items-center gap-2 text-xs text-muted-foreground"><UserRound className="size-4 text-primary/65" />Responsable</div><div className="mt-3 text-sm font-medium text-foreground">{taskRecord.assignedAgent?.name ?? "Sin asignar"}</div></article>
          <article className="nexus-panel rounded-2xl p-5"><div className="flex items-center gap-2 text-xs text-muted-foreground"><CalendarClock className="size-4 text-primary/65" />Fecha objetivo</div><div className="mt-3 text-sm font-medium text-foreground">{taskRecord.due_date ?? "Sin fecha"}</div></article>
          {taskRecord.conversation_id ? <Link href={`/app/conversaciones/${taskRecord.conversation_id}`} className="nexus-panel flex items-center gap-3 rounded-2xl p-5 text-sm text-secondary-foreground hover:border-primary/20"><MessageSquareText className="size-4 text-primary/70" />Abrir conversación de origen</Link> : null}
        </aside>
      </section>

      <section className="nexus-panel mt-4 rounded-2xl p-5 sm:p-6"><div className="flex items-center gap-2"><GitMerge className="size-4 text-primary/70" /><div className="nexus-kicker">Dependencias</div></div>{taskRecord.dependencies?.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2">{taskRecord.dependencies.map((dependency) => <Link key={dependency.id} href={`/app/tareas/${dependency.id}`} className="rounded-xl border border-border p-4"><div className="text-sm text-foreground">{dependency.title}</div><div className="mt-1 text-xs text-muted-foreground/80">{TASK_STATUS_LABELS[dependency.status]}</div></Link>)}</div> : <p className="mt-4 text-sm text-muted-foreground/80">Esta tarea no depende de otra.</p>}</section>

      <section className="nexus-panel mt-4 rounded-2xl p-5 sm:p-6"><div className="flex items-center justify-between gap-4"><div><div className="nexus-kicker">Entregables</div><h2 className="mt-2 text-base font-semibold text-foreground">Artefactos vinculados</h2></div><span className="font-mono text-xs text-primary">{artifacts.length}</span></div>{artifacts.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{artifacts.map((artifact) => <Link key={artifact.id} href={`/app/artefactos/${artifact.id}`} className="rounded-xl border border-border p-4"><div className="text-sm font-medium text-foreground">{artifact.title}</div><div className="mt-2 text-xs text-muted-foreground/80">v{artifact.current_version_number} · {artifact.status}</div></Link>)}</div> : <p className="mt-4 text-sm text-muted-foreground/80">Todavía no hay artefactos relacionados.</p>}</section>
