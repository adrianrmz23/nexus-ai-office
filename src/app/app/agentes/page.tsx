    ...
    <div className="mx-auto max-w-7xl pb-20 lg:pb-0">
      ...
      <section className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <article key={item.label} className="nexus-panel rounded-2xl p-5">
            <div className="font-mono text-[0.62rem] tracking-[0.14em] text-muted-foreground/80 uppercase">{item.label}</div>
      ...
      <section className="nexus-panel mt-5 rounded-2xl p-4 sm:p-5">
        <form className="grid gap-3 lg:grid-cols-[1fr_13rem_13rem_auto]">
          ...
          <select name="role" defaultValue={role} aria-label="Filtrar por rol" className="nexus-focus h-11 rounded-lg border border-input bg-card px-3.5 text-sm text-foreground">
            <option value="all">Todos los roles</option>
            {AGENT_ROLES.map((item) => <option key={item} value={item}>{AGENT_ROLE_LABELS[item]}</option>)}
          </select>
          <select name="status" defaultValue={status} aria-label="Filtrar por estado" className="nexus-focus h-11 rounded-lg border border-input bg-card px-3.5 text-sm text-foreground">
