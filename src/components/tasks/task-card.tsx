...
export function TaskCard({ task }: { task: TaskRecord }) {
  return (
    <Link href={`/app/tareas/${task.id}`} className="nexus-focus block rounded-xl border border-white/[0.06] bg-[#0b1219] p-4 transition-colors hover:border-primary/20 hover:bg-primary/[0.018]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="line-clamp-2 text-sm font-medium leading-6 text-slate-200">{task.title}</div>
          <div className="mt-1 truncate text-xs text-slate-600">{task.project?.name}</div>
        </div>
        <span className={cn(
          ...
          task.priority === "low" && "border-slate-400/10 bg-slate-400/[0.03] text-slate-500",
        )}>{TASK_PRIORITY_LABELS[task.priority]}</span>
      </div>
      {task.description ? <p className="mt-3 line-clamp-2 text-xs leading-5 text-slate-600">{task.description}</p> : null}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-[0.62rem] text-slate-600">
